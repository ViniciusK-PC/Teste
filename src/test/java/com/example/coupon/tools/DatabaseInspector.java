...
public class DatabaseInspector {
    public static void main(String[] args) {
        String uri = "mongodb+srv://viniciuskb35_db_user:0bImSAypxl6CZhRj@cluster.j1abcw7.mongodb.net/coupon_db?retryWrites=true&w=majority";
        ...
        try (MongoClient mongoClient = MongoClients.create(uri)) {
            ...
            List<Document> coupons = database.getCollection("coupons").find().limit(10).into(new ArrayList<>());
            
            if (coupons.isEmpty()) {
                System.out.println(" >> Nenhum cupom encontrado.");
            } else {
                for (Document doc : coupons) {
                    System.out.printf(" ID: %s | Codigo: %-7s | Valor: %-8s | Expira: %s%n",
                            doc.get("_id"),
                            doc.get("code"),
